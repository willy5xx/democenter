...
class ConfigManager {
  constructor(options = {}) {
    ...
    console.log(`🎬 Hardware encoder: ${this.selectedEncoder.displayName} (~${this.selectedEncoder.latencyMs}ms latency)`);
  }
...
`;
      ...
      console.log(`✅ Generated go2rtc.yaml with ${Object.keys(streams).length} streams`);
  ...
  async stopGo2rtc() {
    try {
      ...
      if (!pid) {
        console.log('ℹ️  go2rtc is not running');
        return { success: true, message: 'Not running' };
      }

      console.log(`🛑 Stopping go2rtc (PID: ${pid})...`);
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (!stillRunning) {
          console.log('✅ go2rtc stopped gracefully');
          return { success: true, message: 'Stopped gracefully' };
        }
      }

      // If still running, force kill
      console.log('⚠️  Forcing go2rtc shutdown...');
      process.kill(pid, 'SIGKILL');
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      console.log('✅ go2rtc stopped (forced)');
  ...
  async startGo2rtc() {
    try {
      ...
      const pid = await this.findGo2rtcProcess();
      if (pid) {
        console.log(`✅ go2rtc started (PID: ${pid})`);
  ...
  async restartGo2rtc() {
    ...
    try {
      ...
      console.log('🔄 Restarting go2rtc...');
      
      await this.stopGo2rtc();
      await new Promise(resolve => setTimeout(resolve, 1000)); // Brief pause
      await this.startGo2rtc();
      
      console.log('✅ go2rtc restarted successfully');
  ...
  async updateConfiguration({ debounceMs = 2000 } = {}) {
    ...
    return new Promise((resolve, reject) => {
      this.restartTimeout = setTimeout(async () => {
        try {
          console.log('\n' + '='.repeat(60));
          console.log('🔧 CONFIGURATION UPDATE TRIGGERED');
