 ...
 * 
 * Now with automatic hardware encoder detection for lower latency!
 * 
 * Usage:
 *   node backend/scripts/generate-go2rtc-config.js [--output path/to/go2rtc.yaml]
 *   node backend/scripts/generate-go2rtc-config.js --software  # Force software encoding
 */
...
import { detectHardwareEncoder } from './detect-hardware-encoder.js';
...
const forceSoftware = args.includes('--software');

console.log('🔧 Generating go2rtc.yaml from database...\n');

// Detect hardware encoder
const encoderInfo = detectHardwareEncoder();
const selectedEncoder = forceSoftware 
  ? encoderInfo.available.find(e => e.name === 'software')
  : encoderInfo.best;

console.log(`🎬 Encoder: ${selectedEncoder.displayName} (${selectedEncoder.encoder})`);
console.log(`   Expected transcoding latency: ~${selectedEncoder.latencyMs}ms`);
if (encoderInfo.hasHardware && !forceSoftware) {
  console.log('   ✅ Hardware acceleration enabled!');
} else if (forceSoftware) {
  console.log('   ⚠️  Software encoding forced via --software flag');
}
console.log('');
...
sites.forEach((site, index) => {
  ...
  // Build FFmpeg command with detected hardware encoder
  const inputArgs = '-fflags nobuffer+genpts -flags low_delay -probesize 32 -analyzeduration 0';
  
  // FFmpeg stream with hardware-accelerated encoding (or optimized software fallback)
  streams[dewarpedStreamName] = [
    `ffmpeg:${mainStreamName}#video=h264#raw=${inputArgs} -vf ${filterChain} ${selectedEncoder.args}`
  ];
});
...
# Encoder: ${selectedEncoder.displayName} (${selectedEncoder.encoder})
# Expected transcoding latency: ~${selectedEncoder.latencyMs}ms
#
# ⚠️  WARNING: Manual changes to this file may be overwritten!
# To modify camera configuration, update the database via the API or calibration tool.
#
# Regenerate with: cd backend && node scripts/generate-go2rtc-config.js
# Force software:  cd backend && node scripts/generate-go2rtc-config.js --software
#
...
console.log(`✅ Generated go2rtc.yaml at: ${outputPath}\n`);

// Show stream summary
console.log('📡 Configured streams:');
